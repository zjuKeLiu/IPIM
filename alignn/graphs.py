...
def prepare_line_graph_batch(
    ...
    batch = (
        (
            ...
            [x.to(device, non_blocking=non_blocking) for x in l],
        ),        
        t.to(device, non_blocking=non_blocking),
    )
...
class StructureDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        self.lattice = torch.tensor(self.lattice).type(
            torch.get_default_dtype()
        )
    ...
    def collate(samples: List[Tuple[dgl.DGLGraph, torch.Tensor, torch.Tensor]]):
        ...
        return batched_graph, torch.tensor(lattice), torch.tensor(labels)
    ...
    def collate_line_graph(
        ...
        if len(labels[0].size()) > 0:
            return batched_graph, batched_line_graph, torch.stack(lattice), torch.stack(labels)
        else:
            # This entry
            return batched_graph, batched_line_graph, torch.stack(lattice, 0), torch.tensor(labels)
