...
def lattice_process(one_data_d):
    one_data = one_data_d['atoms']
    
    ind = np.argsort(one_data['abc'])
    max_in = ind[0]
    second_in = ind[1]
    third_in = ind[2]

    [a,b,c] = one_data['abc'][max_in], one_data['abc'][second_in], one_data['abc'][third_in]
    alpha = math.radians(one_data['angles'][max_in])
    beta = math.radians(one_data['angles'][second_in])
    gamma = math.radians(one_data['angles'][third_in])
    lattice = np.array([a,0,0, b*math.cos(gamma),b*math.sin(gamma),0, c*math.cos(beta), c*(math.cos(alpha)-math.cos(beta)*math.cos(gamma))/math.sin(gamma), c*math.sqrt(1+2*math.cos(alpha)*math.cos(beta)*math.cos(gamma)-(math.cos(alpha))**2-(math.cos(beta))**2-(math.cos(gamma))**2)/math.sin(gamma)])
    return lattice
...
def get_train_val_loaders(
    ...
    if (
        ...
        lattice_train = np.array([lattice_process(one_data) for one_data in dataset_train])
        #print("****************************************************************")
        #print("lattice_train.shape", lattice_train.shape)
        #print("****************************************************************")
        dataset_val = [dat[x] for x in id_val]
        lattice_val = np.array([lattice_process(one_data) for one_data in dataset_val])
        dataset_test = [dat[x] for x in id_test]
        lattice_test = np.array([lattice_process(one_data) for one_data in dataset_test])
